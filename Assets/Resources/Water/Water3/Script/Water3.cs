...
public class Water3 : MonoBehaviour {
    ...
    void Start() {
        ...
        float x = transform.position.x - (ScaleX / 2);
        float y = transform.position.y;

        float top = ScaleY + y;
        float bottom = -ScaleY + y;

        SpawnWater(x, ScaleX, top, bottom);
    }
    ...
    public void SpawnWater(float Left, float Width, float Top, float Bottom) {
        ...
        int edgecount = Mathf.RoundToInt(Width) * 10;
        ...
        for (int i = 0; i < edgecount; i++) {
            ...
            //Set the position and scale to the correct dimensions
            colliders[i].transform.position = new Vector3(Left + Width * (i + 0.5f) / edgecount, (Top - ((Top - bottom) / 2)), 0);
            colliders[i].transform.localScale = new Vector3(Width / edgecount, 1, 1);
